...
- **Production deployment**: live demo at [pulse.odieyang.com](https://pulse.odieyang.com) (CORS allowlist already includes this origin).
...
- **Hosting target unclear**: where `pulse.odieyang.com` is actually deployed (VPS / Fly.io / Render / your own server?) needs to be filled in. The CORS list contains `pulse-frontend-tawny.vercel.app`, hinting at a former Vercel-hosted frontend split — confirm whether the current deploy is "single self-hosted jar" or "split frontend + backend".
